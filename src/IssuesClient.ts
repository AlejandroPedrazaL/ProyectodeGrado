...
export default class IssuesClient {
  ...
  public static async postProjectIssues(projectId: string): Promise<IssuesList | undefined> {
    const accessToken = await IssuesClient.getAccessToken();
    let url = `https://api.bentley.com/issues/`;
    const options = {
      method: "POST",
      headers: {
      ...
      body:{
        "subject": "Prueba",
        "description": "Desde codigo",
        "formId": "apThWznqk063WWIJjrWx6TMVWH-3zRFNujk1rXsdzsw",
    ...
    return fetchApi<IssuesList>(url, );
  }
